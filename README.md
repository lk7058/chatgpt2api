...
- **注册域名白名单**：可配置仅允许指定邮箱域名注册（如机构/企业域名）
...
- 额度流水明细与汇总统计（管理员可查看全站流水）
- 模型额度权重配置（按模型扣减，支持前缀匹配）

### 在线画图工作台

- 内置画图界面，支持文生图、图生图编辑与多图组图编辑
- 支持 `gpt-image-2`、`codex-gpt-image-2`、`auto`、`gpt-5` 系列模型
- **任务队列**：多图提交自动排队，实时四阶段进度（排队 → 启动 → 生成 → 取图）展示
- **任务取消**：用户可取消自己的排队/生成中任务，管理员可批量或一键取消全部
- 图片会话历史回看、服务端缓存、历史图片在首页聚合展示

### 图片生成与编辑 API（OpenAI 兼容）
...
- 支持 `url` / `b64_json` 两种返回格式（b64 直传本地化存储，绕开海外 CDN 慢速下载）
- 可接入自定义 OpenAI 兼容 API 中转（第三方模型自动路由、图片镜像回本地）
...
- **排队任务管理**：查看所有用户的排队/生成中任务，勾选批量取消或一键取消全部
- **日志管理**：调用/账号/安全/签到/额度分类日志，支持日期筛选、按用户过滤、详情与图片预览
- **用户日志**：用户管理页一键跳转查看指定用户的全部操作日志
- **登录审计**：用户管理展示每个账号的最近登录时间与登录 IP

### 公告与首页

- 弹窗公告 + 首页横幅公告（后台可配置）
- 首页聚合展示：公告、个人信息与额度、历史生成图片

### 系统管理

- 用户管理：创建（邮箱）、额度分配、启停、重置密码、删除、查看登录信息与日志
- 图片管理：缩略图、WebDAV 存储、标签、批量清理、存储压缩
- 定时备份：支持 Cloudflare R2 存储、可选加密、自动轮替
- 第三方 API / sub2api / CPA 池 / FlareSolverr 配置
- 充值卡生成与管理、用户密钥管理
- 操作日志与健康检查（仅管理员可见）
